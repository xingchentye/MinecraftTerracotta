...
public class MinecraftTerracottaClient implements ClientModInitializer {
    private static final Logger LOGGER = LoggerFactory.getLogger("minecraftterracotta-fabric");
    ...
    private static void showRoomCodeInChat(String roomCode) {
        ...
        Text msg = Text.literal("[Terracotta] 房间已创建！房间号: ").formatted(Formatting.GREEN);
        Text code = Text.literal(roomCode).styled(style -> style
        ...
        client.inGameHud.getChatHud().addMessage(msg.copy().append(code));
    }
    ...
    public void onInitializeClient() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (!hasAutoStarted && client.currentScreen instanceof TitleScreen) {
                hasAutoStarted = true;
                if (FabricConfig.isAutoStartBackend() && !TerracottaApiClient.hasDynamicPort()) {
